...
Функция ДанныеДляФормированияГрафа(ПараметрыФормирования) Экспорт
	...
	ПараметрыФормирования.Вставить("КешОформлений", Новый Соответствие);
...
Процедура ДобавитьСКДТекущийУровеньДерева(ПараметрыФормирования, ТекущийУровень, ТекущаяСтруктураСКД)
	...
	Для каждого Строка Из  СтрокиДерева Цикл
		...
		Если ТипЗнч(Строка.Оформление) = Тип("Строка") 
			И ЗначениеЗаполнено(Строка.Оформление) Тогда
			НовоеПолеГруппировкиСКД(ГруппировкаКомпоновкиДанных, Строка.Оформление);
		КонецЕсли;	
...
Процедура ДобавитьСвязиВСКД(ПараметрыФормирования, ТекущаяСтруктураСКД)
	...
	Для Каждого Строка Из ОписаниеРебер Цикл
		...
		Если ЗначениеЗаполнено(Строка.Направление) Тогда
			ДобавитьПоляЗаголовкаВГруппировку(ПараметрыФормирования, ГруппировкаКомпоновкиДанных, Строка.Направление);
		КонецЕсли;
		Если ТипЗнч(Строка.Оформление) = Тип("Строка") 
			И ЗначениеЗаполнено(Строка.Оформление) Тогда
			НовоеПолеГруппировкиСКД(ГруппировкаКомпоновкиДанных, Строка.Оформление);
...
Процедура ДобавитьСвязь(ПараметрыФормирования, Строка, ДанныеРезультата)
	...
	НоваяСтрока.Оформление = ОформлениеСтроки(ПараметрыФормирования, СтрокаСвязи.Оформление, Строка);
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(СтрокаСвязи.Направление) Тогда
		НоваяСтрока.Направление = Строка[СтрокаСвязи.Направление];
	КонецЕсли;
КонецПроцедуры

Функция ОформлениеСтроки(ПараметрыФормирования, ЗначениеОформления, Строка)
	Если ТипЗнч(ЗначениеОформления) =  Тип("СправочникСсылка.граф_ОформленияШаблонов") Тогда
		Возврат ЗначениеОформления;
	КонецЕсли;	
	КешОформлений = ПараметрыФормирования.КешОформлений;
	ИмяОформления = Строка[ЗначениеОформления];
	НайденноеЗначение = КешОформлений[ИмяОформления];
	Если НайденноеЗначение = Неопределено Тогда
		НайденноеЗначение = Справочники.граф_ОформленияШаблонов.НайтиПоНаименованию(ИмяОформления);
		КешОформлений.Вставить(ИмяОформления, НайденноеЗначение);
	КонецЕсли;
...
Функция НовыйУзел(ПараметрыФормирования, Строка, ДанныеРезультата) 
	...
	КонецЕсли;	
	НоваяСтрока.Оформление = ОформлениеСтроки(ПараметрыФормирования, СтрокаГруппировки.Оформление, Строка);
	Возврат НоваяСтрока;
КонецФункции
...
Функция ОписаниеРебра(Строка, ОптимизироватьСвязи, СуществующиеСвязи, КешОформлений) 
	...
	Иначе
		ДобавитьИсточникИприемникПоНаправлению(Строка.ИдентификаторИсточника, Строка.ИдентификаторПриемника, Строка, Результат)
	КонецЕсли;
	...
	РазвернутьОптимизированныеСвязи(Результат, СвойстваОформления);
...
Функция ДобавитьИсточникИприемникПоНаправлению(Источник, Приемник, Строка, Результат)
	Если Строка.Направление = -1 Тогда
			Результат.Вставить("source", Приемник);
			Результат.Вставить("target", Источник);
		иначе
			Результат.Вставить("source", Источник);
			Результат.Вставить("target", Приемник);	
		КонецЕсли;
КонецФункции

Процедура РазвернутьОптимизированныеСвязи(Результат, СвойстваОформления)
	Если Результат.Свойство("reverse") = Ложь Тогда
		Возврат;
	КонецЕсли;
	КопияСвойствФикс = Новый ФиксированнаяСтруктура(СвойстваОформления);
	КопияСвойств = Новый Структура(КопияСвойствФикс);	
	ОписаниеJSON = КопияСвойств.ОписаниеJSON;
	ОписаниеJSON = СтрЗаменить(ОписаниеJSON,"endArrow","endArrow_");	
	ОписаниеJSON = СтрЗаменить(ОписаниеJSON,"startArrow","startArrow_");
	ОписаниеJSON = СтрЗаменить(ОписаниеJSON,"endArrow_","startArrow");	
	ОписаниеJSON = СтрЗаменить(ОписаниеJSON,"startArrow_","endArrow");
	КопияСвойств.ОписаниеJSON = ОписаниеJSON;
	СвойстваОформления = КопияСвойств;
КонецПроцедуры
...
Процедура ОпределитьИсточникИПриемникПоОптимизации(Результат, Строка, СуществующиеСвязи)
	...
	Если НаправлениеСвязи = "Прямое" Тогда
		ДобавитьИсточникИПриемникПоНаправлению(Строка.ИдентификаторИсточника, Строка.ИдентификаторПриемника, Строка, Результат);
	Иначе
		ДобавитьИсточникИПриемникПоНаправлению(Строка.ИдентификаторПриемника, Строка.ИдентификаторИсточника, Строка, Результат);
		Результат.Вставить("reverse");
	КонецЕсли;
...
Функция ОписаниеРегистрацииНовыхТипов(ДанныеДляГрафа) 
	...
	ТекстРегистрации = Справочники.граф_ШаблоныЭлементов.ТекстРегистрацииРасширяемыхТипов(ПользовательскиеТипы);
...
Функция ТолькоПользовательскиеТипы(Оформления)
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.граф_ОформленияШаблонов КАК граф_ЭлементыГрафов
		|ГДЕ
...
Функция КешОформлений()
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.граф_ОформленияШаблонов КАК граф_ОформленияЭлементовГрафа
		|ГДЕ
