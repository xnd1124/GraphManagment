...
Функция ОписаниеРеберJSON(ДанныеДляГрафа, ПараметрыФормирования, КешОформлений) 
	...
	НаправленияСвязей = Новый Соответствие;
	...
	Для Каждого Строка Из Ребра Цикл
		ОписаниеРебра = ОписаниеРебра(Строка, ОптимизироватьСвязи, НаправленияСвязей, КешОформлений);
		Результат.Добавить(ОписаниеРебра);
	КонецЦикла;
	Возврат Результат;
КонецФункции

Функция ОписаниеРебра(Строка, ОптимизироватьСвязи, НаправленияСвязей, КешОформлений) 
	Результат = Новый Структура;
	Если ОптимизироватьСвязи Тогда
		ОпределитьИсточникИПриемникПоОптимизации(Результат, Строка, НаправленияСвязей);
	Иначе
...
Процедура ОпределитьИсточникИПриемникПоОптимизации(Результат, Строка, НаправленияСвязей)
	...
	НаправлениеСвязи = НаправленияСвязей.Получить(КлючПоиска);
	Если НаправлениеСвязи = Неопределено Тогда
		КоличествоСвязей = НаправленияСвязей.Количество();
		Если КоличествоСвязей % 2 = 0 Тогда
			НаправлениеСвязи = "Прямое";
		Иначе
			НаправлениеСвязи = "Обратное";
		КонецЕсли;
		НаправленияСвязей.Вставить(КлючПоиска, НаправлениеСвязи);
	КонецЕсли;	
