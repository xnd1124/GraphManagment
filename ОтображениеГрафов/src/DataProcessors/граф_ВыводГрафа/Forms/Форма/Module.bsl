...
Процедура ДобавитьОписаниеСвязей(Результат)
	...
	ШаблонСвязи = """%1"" -> ""%2"" [label=""%3""]";
	Для каждого Строка Из СвязиУзлов Цикл           
		Заголовок = СтрСоединить(СтрРазделить(Строка.Заголовок,";"), "\n");
		...
		Если Строка.ВесСвязи > 0 Тогда
			Результат = Результат + Символы.ПС + СтрШаблон(ШаблонСвязи, Строка.УзелНачала, Строка.УзелКонца, Заголовок);
		ИначеЕсли Строка.ВесСвязи < 0 Тогда
			Результат = Результат + Символы.ПС + СтрШаблон(ШаблонСвязи, Строка.УзелКонца, Строка.УзелНачала, Заголовок);	
