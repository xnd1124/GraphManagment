...
Функция НайтиИдентификаторИДобавитьУзел(УзлыГрафа, Строка, НазначениеКолонок, ИдентификаторРодителя) 
	Для Счетчик = 0 По НазначениеКолонок.КоличествоКолонокВсего - 1 Цикл 
		...
		Если ТекущееЗначениеЗаполнено 
			...
			ДобавитьУзел(УзлыГрафа, ТекЗначение, ИдентификаторРодителя);  
...
&НаСервере
Процедура ДобавитьУзел(УзлыГрафа, Идентификатор, ИдентификаторРодителя)         
...
Функция НазначениеКолонок(ДеревоСКД)
	...
	Результат.Вставить("КолонкаЭлементГрафа", КолонкиСодержащие("ЭлементГрафа", ДеревоСКД));
...
&НаСервере
Функция КолонкиСодержащие(ИскомыеФрагменты, ДеревоСКД) 
	...
	Для каждого Колонка Из ДеревоСКД.Колонки Цикл
		...
		Для каждого Фрагмент Из МассивФрагментов Цикл
			Если  СтрНайти(_заголовок, СокрЛП(Фрагмент)) > 0 
				ИЛИ СтрНайти(_Имя, СокрЛП(Фрагмент)) Тогда
...
Процедура ВывестиГраф()
	...
	ШаблонТекстаСтраницы = СтрЗаменить(ШаблонТекстаСтраницы, "//данные графа", ОписаниеГрафаJSON);
