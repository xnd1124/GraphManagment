...
Процедура РебраОформлениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораТипаОформления", ЭтотОбъект, ТекущиеДанные);	
...
&НаКлиенте
Процедура УстановитьПараметрыВыбораУзла()
	ТекущиеДанные = Элементы.Узлы.ТекущиеДанные;
	ЗначенияОтбора = Новый Массив;
	Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
		ЗначенияОтбора.Добавить(ПредопределенноеЗначение("Перечисление.граф_КомпонентыГрафа.Контейнер"));
	Иначе
		ЗначенияОтбора.Добавить(ПредопределенноеЗначение("Перечисление.граф_КомпонентыГрафа.Вершина"));
	КонецЕсли;
	ЗначенияОтбора = Новый ФиксированныйМассив(ЗначенияОтбора);
	НовыйПараметр = Новый ПараметрВыбора("Отбор.Компонент", ЗначенияОтбора);
	НовыйМассив = Новый Массив;
	НовыйМассив.Добавить(НовыйПараметр);

	НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
	Элементы.УзлыОформление.ПараметрыВыбора = НовыеПараметры;
КонецПроцедуры
...
Процедура ПослеВыбораТипаОформления(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатВыбора.Значение = "Элемент оформления" Тогда
		...
		ОткрытьФорму("Справочник.граф_ОформленияЭлементовГрафа.ФормаВыбора", , ЭтотОбъект, , , , Оповещение);
	Иначе
...
Процедура ПослеВыбораОформления(РезультатВыбора, ДополнительныеПараметры) Экспорт
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;	
	ДополнительныеПараметры.Оформление = РезультатВыбора;
