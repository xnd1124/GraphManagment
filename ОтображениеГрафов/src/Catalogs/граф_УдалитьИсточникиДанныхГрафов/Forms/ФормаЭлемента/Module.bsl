...
Процедура НастроитьСКД()
	Для Каждого Макет Из Метаданные.Справочники.граф_УдалитьИсточникиДанныхГрафов.Макеты Цикл
	...
	Если Параметры.ЗначениеКопирования.Пустая() Тогда
		СхемаИНастройки = Справочники.граф_УдалитьИсточникиДанныхГрафов.ОписаниеИСхемаКомпоновкиДанныхГрафаПоИмениМакета(Объект.Ссылка,
			Объект.ИмяШаблонаСКД);
	Иначе
		СхемаИНастройки = Справочники.граф_УдалитьИсточникиДанныхГрафов.ОписаниеИСхемаКомпоновкиДанныхГрафаПоИмениМакета(Параметры.ЗначениеКопирования, 
...
Процедура ИмяШаблонаСКДПриИзмененииНаСервере()
	АдресСхемыКомпоновкиДанных = ПоместитьВоВременноеХранилище(
		Справочники.граф_УдалитьИсточникиДанныхГрафов.ПолучитьМакет(Объект.ИмяШаблонаСКД), Новый УникальныйИдентификатор());
	АдресНастроекКомпоновкиДанных = "";
КонецПроцедуры
...
Функция АдресаСхемыКомпоновкиДанныхИНастроекВоВременномХранилище()
	...
	Возврат Справочники.граф_УдалитьИсточникиДанныхГрафов.АдресаСхемыКомпоновкиДанныхИНастроекВоВременномХранилище(Объект);
