...
Процедура ЗакешироватьТекстТипаЭлемента()
	Если Объект.ТипЭлемента = Перечисления.граф_ТипЭлементаГрафа.Ребро Тогда
		КешированныйТекстТипаЭлемента = ПолучитьОбщийМакет("граф_ШаблонРеброПоУмолчанию").ПолучитьТекст();
	ИначеЕсли Объект.ТипЭлемента = Перечисления.граф_ТипЭлементаГрафа.Вершина Тогда
		КешированныйТекстТипаЭлемента = ПолучитьОбщийМакет("граф_ШаблонВершинаПоУмолчанию").ПолучитьТекст();
	КонецЕсли;
КонецПроцедуры
